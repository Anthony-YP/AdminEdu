...
function formatearFecha(fecha) {
    if (!fecha) return "—";
    // Se arma la fecha en UTC explícito para evitar el corrimiento de un día
    // que produce `new Date("YYYY-MM-DD")` al interpretarla en la zona local.
    ...
    return (
        <div className="space-y-6 animate-fadeIn max-w-3xl mx-auto">
            <div>
                ...
                <p className="text-sm text-gray-500">Vista general de tu cuenta y tus datos personales.</p>
            </div>

            {error && <Alert variant="error">{error}</Alert>}
            {success && <Alert variant="success">{success}</Alert>}

            {/* Encabezado con avatar, nombre y roles */}
            ...
            {!persona ? (
                <Alert variant="info">
                    Aún no tienes datos personales registrados. Solicita a la administración que complete tu información.
                </Alert>
            ) : (
